package it.poker.PokerOnline.filter;
...
public class CheckAuthFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		System.out.println("current path: " + pathAttuale);
		...
		if (!isInWhiteList) {
			User utenteInSession = (User) httpRequest.getSession().getAttribute("userInfo");
			// intanto verifico se utente in sessione
			if (utenteInSession == null) {
				System.out.println("FILTER user not in session: redirecting from " + pathAttuale + " to " + httpRequest.getContextPath());
